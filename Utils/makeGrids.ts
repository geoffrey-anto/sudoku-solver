...
// export default function splitIntogrids(img: Image, rows: number, columns: number){
//     const {bytes, width, height} = img;
//     const sizeCol = width/columns;
//     const sizeRow = height/rows;
//     const grids: Image[] = new Array(rows*columns);
//     for(let i=0; i<rows; i++){
//         for(let j=0; j<columns; j++){
//             const grid = Image.withSize(sizeCol, sizeRow);
//             for(let x=0; x<sizeRow; x++){
//                 for(let y=0; y<sizeCol; y++){
//                     const index = (i*sizeRow+y)*width + j*sizeCol+x;
//                     grid.bytes[x*sizeCol+y] = bytes[index];
//                 }
//             }
//             grids[i*columns+j] = grid;
//         }
//     }
//     return grids;
// }
...
import Image from "./Image";
import { getConnectedComponent } from "./largestConnectedConmonent";
